...
MODULE module_sf_noahlsm    !zgf modified according to wuxb  2018.09.10    !zgf add line 3857，#define 1, zgf delete line 3862
...
! 2 -> New parametric scheme,   //have gravel & bedrock
  INTEGER :: OPT_TCND = 2  !(suggested 2 gravel)
    ...
    real :: SLPCATS   !zgf modified INTEGER to real, 2018.09.10
    ...
    CONTAINS
    !
    !zgf add stype line 74,192,284   2018.09.10
    ...
    INTEGER, DIMENSION(1:NSOIL), INTENT(IN) :: stype    !zgf add 2018.09.10
    ...
    REAL :: Z0MIN,     Z0MAX
    INTEGER :: stpnum !zgf add 2018.09.10
    ! ----------------------------------------------------------------------
...
! PENMAN.
        ...
        ESNOW = 0.0
        IF (SNEQV  == 0.0) THEN
            !zgf add stype in line 733,  2018.09.10
            ...
            ETA_KINEMATIC = ETA
         ELSE
            !zgf add stype in line 747,  2018.09.10
        ...
        IF (NROOT >= 2) THEN
            DO K = 2,NROOT
                ...
                CALL REDSTP (stpnum,  PSISAT, BEXP,DKSAT,DWSAT,       &
                    SMCMAX,SMCWLT,SMCREF,SMCDRY,F1,QUARTZ)
                !write(*,'("SMCMAX=''", F11.7, "''")') SMCMAX   !zgf add, 2018.09.10
    ...
    ! ----------------------------------------------------------------------
    !zgf removed SUBROUTINE FRH2O to module_optimal
    ! ----------------------------------------------------------------------
    !zgf add stype  2018.09.10
    ...
    INTEGER              :: I, K
    INTEGER              :: stpnum !zgf add 2018.09.10
    REAL                 :: DKSAT,DWSAT,SMCDRY, SMCREF,SMCWLT  !zgf add 2018.09.10
    REAL                 :: BEXP, F1,PSISAT,QUARTZ, SMCMAX     !zgf add 2018.09.10
    REAL, INTENT(IN)     :: CSOIL, DF1, DT, TBOT,YY,ZZ1, ZBOT  !zgf add 2018.09.10
    !zgf delete 2015.10.2
    ...
    INTEGER, INTENT(IN)  :: ISURBAN
    INTEGER              :: K
    INTEGER              :: stpnum !zgf added,2018.09.10
    ...
    INTEGER, DIMENSION(1:NSOIL),INTENT(IN)  :: stype !zgf added,2018.09.10
	...
	!REAL, INTENT(IN)      :: BEXP,CSOIL,DF1,DT,F1,PSISAT,QUARTZ,     &  !zgf delete, 2018.09.10
	!                            SMCMAX, SMCWLT, TBOT,YY, ZBOT,ZZ1       !zgf delete, 2018.09.10

  REAL, INTENT(IN)      :: BEXP,CSOIL,DF1,DT,F1,PSISAT,QUARTZ,     &   !zgf add, 2018.09.10
	SMCMAX, SMCWLT, TBOT,YY, ZBOT,ZZ1   !zgf add, 2018.09.10
	REAL, INTENT(INOUT)   :: T1
	REAL, INTENT(OUT)     :: SSOIL
	REAL, DIMENSION(1:NSOIL), INTENT(IN)    :: SMC,ZSOIL
	INTEGER, DIMENSION(1:NSOIL), INTENT(IN)    :: stype   !zgf add stype, 2018.09.10
	REAL, DIMENSION(1:NSOIL), INTENT(INOUT) :: SH2O
  ...
  REAL                  :: BEXP, DKSAT,DWSAT, SMCMAX, SMCWLT
	...
	REAL :: FAC2
	REAL :: FLIMIT
	REAL :: F1,QUARTZ,SMCDRY,SMCREF,PSISAT  !zgf add stype 2018.09.10
...
!zgf add stype 2018.09.10
  ...
  REAL, INTENT(IN)          :: DT, EDIR, FRZX,  KDT, PCPDRP, SLOPE
	...
	INTEGER, DIMENSION(1:NSOIL)  :: stype !zgf add 2018.09.10
...
!zgf add 2018.09.10
		...
		FCR = 1.
        IF(OPT_INF == 1) THEN   !zgf add 2018.09.10
		  IF (DICE >  1.E-2) THEN
...
!zgf update 2018.09.10
        IF(OPT_INF == 2)  THEN
            ...
            !CALL WDFCND_EI10 (WDF,WCND,SMCWXB,SMCMAX,BEXP,DKSAT,DWSAT,SICEMAX,SICE(1))
        END IF
		IF(OPT_INF == 1) THEN
...
!zgf update 2018.09.10
            IF(OPT_INF == 2) THEN
                ...
                !CALL WDFCND_EI10 (WDF2,WCND2,SMCWXB2,SMCMAX,BEXP,DKSAT,DWSAT,SICEMAX,SICE(K))
            END IF
			IF(OPT_INF == 1) THEN
...
!zgf update 2018.09.10
            IF(OPT_INF == 2) THEN
                ...
                !CALL WDFCND_EI10 (WDF2,WCND2,SMCWXB3,SMCMAX,BEXP,DKSAT,DWSAT,SICEMAX,SICE(K))
            END IF
			IF(OPT_INF == 1) THEN
  ...
  !zgf add type, 2018.09.10
	...
	REAL, INTENT(IN)          :: CMCMAX, DT     !zgf modified 2018.09.10
	REAL                      :: SMCMAX   !zgf modified 2018.09.10
	...
	REAL, DIMENSION(1:NSOIL)  :: CIin
	INTEGER, DIMENSION(1:NSOIL)  :: stype  !zgf  2018.09.10
	REAL                      :: DDZ, RHSCT, STOT, WPLUS

	REAL     :: BEXP,DKSAT,DWSAT,F1,QUARTZ,SMCDRY,SMCREF,SMCWLT,PSISAT   !zgf add 2018.09.10
	! ----------------------------------------------------------------------
	...
	END SUBROUTINE TBND
! ----------------------------------------------------------------------
!zgf removed SUBROUTINE TDFCND to module_optimal  2018.09.10
	! ----------------------------------------------------------------------
	! ----------------------------------------------------------------------
...
!zgf merge,2018.09.10
	...
	! ----------------------------------------------------------------------
  !zgf removed SUBROUTINE WDFCND to module_optimal  2018.09.10
	! ---------------------------------------------------------------------
